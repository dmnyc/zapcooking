<script lang="ts">
  ...
  import FingerprintSimpleIcon from 'phosphor-svelte/lib/FingerprintSimple';
  import PencilSimpleIcon from 'phosphor-svelte/lib/PencilSimple';
  import { isPasskeyAvailable, backupToPasskey, restoreFromPasskey, createWithPasskey } from '$lib/spark/passkey';
  ...
  let isPasskeyBackingUp = false;
  let passkeyBackedUp = false;
  ...
  onMount(() => {
    ...
    // Check passkey availability and backup status
  ...
  async function handleCreateWithPasskey() {
    if (!BREEZ_API_KEY) {
      errorMessage = 'Breez API key not configured.';
      return;
    }

    isConnecting = true;
    sparkLoadingMessage = 'Creating wallet with passkey...';
    errorMessage = '';

    try {
      await createWithPasskey($userPublickey, BREEZ_API_KEY);

      await connectWallet(4, 'spark');
      await refreshBalance();

      successMessage = 'Passkey wallet created!';
      showAddWallet = false;
      selectedWalletType = null;
    } catch (e) {
      errorMessage = e instanceof Error ? e.message : 'Failed to create passkey wallet';
    } finally {
      isConnecting = false;
      sparkLoadingMessage = '';
    }
  ...
  async function handleRestoreFromPasskey() {
    if (!BREEZ_API_KEY) {
      errorMessage = 'Breez API key not configured. Please contact support.';
      return;
    }
    ...
    isConnecting = true;
    sparkLoadingMessage = 'Restoring from passkey...';
    errorMessage = '';

    try {
      // Decrypt mnemonic from Nostr using passkey PRF
      const mnemonic = await restoreFromPasskey($userPublickey);
      ...
      if (success) {
        ...
        successMessage = 'Wallet restored from passkey!';
        showAddWallet = false;
        selectedWalletType = null;
        sparkRestoreMode = 'options';
      } else {
        errorMessage = 'Failed to connect wallet from passkey';
      }
    } catch (e) {
      errorMessage = e instanceof Error ? e.message : 'Failed to restore from passkey';
    } finally {
      isConnecting = false;
      sparkLoadingMessage = '';
    }
  ...
  async function handleBackupToPasskey() {
    isPasskeyBackingUp = true;
    errorMessage = '';

    try {
      await backupToPasskey($userPublickey);
      successMessage = 'Wallet protected with passkey!';
      saveBackupStatus('passkey');
      showBackupReminder = false;
    } catch (e) {
      errorMessage = e instanceof Error ? e.message : 'Failed to protect with passkey';
    } finally {
      isPasskeyBackingUp = false;
    }
  }
  ...
  function loadBackupStatus() {
    ...
    if (status) {
      ...
      if (status === 'passkey') passkeyBackedUp = true;
    }
  }
  ...
  function saveBackupStatus(method: string) {
    ...
    backupReminderDismissed = true;
    if (method === 'passkey') passkeyBackedUp = true;
  }
...
<PullToRefresh bind:this={pullToRefreshEl} on:refresh={handleRefresh}>
  <div class="max-w-2xl mx-auto py-8 px-4">
              ...
              <div class="flex flex-col gap-2">
                ...
                {#if passkeyAvailable}
                  <button
                    class="flex items-center gap-3 w-full py-2.5 px-4 rounded-xl text-sm font-medium text-left transition-colors hover:bg-white/5 disabled:opacity-40"
                    style="border: 1px solid var(--color-input-border);"
                    on:click={handleBackupToPasskey}
                    disabled={isPasskeyBackingUp}
                  >
                    <FingerprintSimpleIcon size={18} class="text-amber-500 flex-shrink-0" />
                    <span class="text-primary-color">
                      {isPasskeyBackingUp ? 'Setting up...' : 'Protect with passkey'}
                    </span>
                  </button>
                {/if}
              </div>
                  ...
                  <div class="grid grid-cols-2 gap-2">
                    {#if passkeyAvailable}
                      <button
                        ...
                        class:cursor-pointer={!passkeyBackedUp}
                        class:opacity-50={isPasskeyBackingUp}
                        style="background-color: var(--color-bg-primary); border: 1px solid {passkeyBackedUp ? 'rgba(34, 197, 94, 0.4)' : 'var(--color-input-border)'};"
                        on:click={handleBackupToPasskey}
                        disabled={isPasskeyBackingUp || passkeyBackedUp}
                      >
                        {#if passkeyBackedUp}
                          <CheckCircleIcon size={16} class="text-green-500" />
                          Passkey Active
                        {:else}
                          <FingerprintSimpleIcon size={16} />
                          {isPasskeyBackingUp ? 'Setting up...' : 'Passkey'}
                        {/if}
                      </button>
                    {/if}
                    <button
                ...
                {#if sparkRestoreMode === 'options'}
                  ...
                  {:else}
                    <div class="space-y-3 mb-4">
                      {#if passkeyAvailable}
                        <Button
                          on:click={handleCreateWithPasskey}
                          disabled={isConnecting}
                          class="w-full"
                        >
                          <FingerprintSimpleIcon size={16} />
                      ...
                      <div class="space-y-2">
                        {#if passkeyAvailable}
                          <button
                            ...
                            style="background-color: var(--color-bg-primary); border: 1px solid var(--color-input-border);"
                            on:click={handleRestoreFromPasskey}
                            disabled={isConnecting}
                          >
                            <FingerprintSimpleIcon size={16} />
    ...
    <!-- Mnemonic Display Modal (after wallet creation) - removed, replaced by backup reminder banner -->
        ...
        >
          ...
          >
            ...
            {#if walletToDelete.kind === 4 || walletToDelete.kind === 3}
              {#if walletToDelete.kind === 4 && passkeyBackedUp}
                <div
                  class="p-4 rounded-xl mb-4 flex items-center gap-3"
                  style="background-color: rgba(34, 197, 94, 0.08); border: 1px solid rgba(34, 197, 94, 0.3);"
                >
                  <CheckCircleIcon size={18} class="text-green-500 flex-shrink-0" />
                  <span class="text-sm text-green-400">
                    Passkey active — you can restore this wallet anytime.
                  </span>
                </div>
              {:else}
